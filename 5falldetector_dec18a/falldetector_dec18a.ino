...
// ADXL345 I²C Address
#define ADXL345_ADDRESS 0x53       // Default I²C address (SDO to GND)

// Fall Detection Parameters
#define FALL_INTERRUPT_PIN GPIO_NUM_35  // GPIO 35 for fall detection interrupt
#define FALL_THRESHOLD_G 2.0       // Acceleration threshold in g (1.5-2.5g recommended)
#define FALL_DURATION_MS 20        // Detection window in milliseconds (10-30ms)
#define FALL_DEBOUNCE_MS 1000      // Ignore subsequent events for 1 second
#define FALL_AXES_ENABLE 0x70      // Enable X, Y, Z axes (bits 6-4)

// ADXL345 Register Addresses
#define ADXL345_REG_DEVID 0x00     // Device ID (should read 0xE5)
#define ADXL345_REG_THRESH_ACT 0x24    // Activity threshold
#define ADXL345_REG_TIME_ACT 0x25      // Activity time
...
#define ADXL345_RANGE_16G 0x0B     // ±16g range, full resolution
#define ADXL345_RATE_100HZ 0x0A    // 100 Hz output data rate
#define ADXL345_MEASURE_MODE 0x08  // Measurement mode enabled
#define ADXL345_INT_ACTIVITY 0x10  // Activity interrupt bit
...
typedef struct {
  ...
  uint32_t fallEventCount;     // NEW: Total fall events detected
  ...
  uint32_t lastFallTimestamp;  // NEW: Timestamp of last fall (millis)
...
enum MeasurementState {
  ...
  STATE_FALL_DETECTED,     // NEW: Fall event detected
...
void shutdownPeripherals() {
  ...
  if (accelInitialized) {
    ...
    Serial.println("ADXL345 accelerometer powered down");
  }
...
void IRAM_ATTR fallDetectISR() {
  unsigned long currentTime = millis();
  
  // Debounce: ignore if within debounce period
  if (currentTime - lastFallTime > FALL_DEBOUNCE_MS) {
    fallDetected = true;
...
bool initializeADXL345() {
  ...
  // Initialize sensor with I²C
  if (!accel.begin(ADXL345_ADDRESS)) {
    Serial.println("ERROR: ADXL345 not found at address 0x53");
    return false;
  }
  ...
  // Configure range to ±16g for fall detection
  accel.setRange(ADXL345_RANGE_16_G);
  Serial.println("Range set to ±16g");
  
  // Configure data rate to 100 Hz
...
bool configureADXL345FallDetection() {
  Serial.println("Configuring ADXL345 fall detection...");
  
  // Calculate threshold value (62.5 mg/LSB)
  // For 2.0g: 2.0g / 0.0625g = 32 = 0x20
  uint8_t thresholdValue = (uint8_t)(FALL_THRESHOLD_G / 0.0625);
  
  // Calculate time value (1 LSB = 10ms at 100Hz)
  uint8_t timeValue = FALL_DURATION_MS / 10;
  
  Serial.print("Threshold: "); Serial.print(FALL_THRESHOLD_G); Serial.println("g");
  Serial.print("Threshold Register Value: 0x"); Serial.println(thresholdValue, HEX);
  Serial.print("Duration: "); Serial.print(FALL_DURATION_MS); Serial.println("ms");
  Serial.print("Duration Register Value: 0x"); Serial.println(timeValue, HEX);
  
  // Write configuration registers via I²C
  Wire.beginTransmission(ADXL345_ADDRESS);
  Wire.write(ADXL345_REG_THRESH_ACT);
  Wire.write(thresholdValue);
  if (Wire.endTransmission() != 0) {
    Serial.println("ERROR: Failed to write activity threshold");
    return false;
  }
  
  Wire.beginTransmission(ADXL345_ADDRESS);
  Wire.write(ADXL345_REG_TIME_ACT);
  Wire.write(timeValue);
  if (Wire.endTransmission() != 0) {
    Serial.println("ERROR: Failed to write activity time");
    return false;
  }
  
  // Configure activity control (AC-coupled, all axes enabled)
  Wire.beginTransmission(ADXL345_ADDRESS);
  Wire.write(ADXL345_REG_ACT_INACT_CTL);
  Wire.write(0xF0); // AC-coupled, X/Y/Z enabled
  if (Wire.endTransmission() != 0) {
    Serial.println("ERROR: Failed to write activity control");
    return false;
  }
  
  // Enable activity interrupt
  Wire.beginTransmission(ADXL345_ADDRESS);
  Wire.write(ADXL345_REG_INT_ENABLE);
  Wire.write(ADXL345_INT_ACTIVITY);
  if (Wire.endTransmission() != 0) {
  ...
  Wire.beginTransmission(ADXL345_ADDRESS);
  Wire.write(ADXL345_REG_INT_MAP);
  Wire.write(0x00); // All interrupts to INT1
  if (Wire.endTransmission() != 0) {
    Serial.println("ERROR: Failed to map interrupt");
    return false;
  }
  
  Serial.println("ADXL345 fall detection configured successfully");
  return true;
}

void clearADXL345Interrupt() {
  ...
  if (Wire.available()) {
    ...
    Serial.print("Interrupt source cleared: 0x");
...
void setup() {
  ...
  if (!initializeADXL345()) {
    Serial.println("ERROR: ADXL345 initialization failed");
    accelInitialized = false;
  } else {
    accelInitialized = true;
    
    // Configure fall detection
    if (!configureADXL345FallDetection()) {
      Serial.println("ERROR: Fall detection configuration failed");
      accelInitialized = false;
    } else {
      // Configure GPIO 35 as interrupt input
      pinMode(FALL_INTERRUPT_PIN, INPUT);
      
      // Attach interrupt handler (trigger on RISING edge)
      attachInterrupt(digitalPinToInterrupt(FALL_INTERRUPT_PIN), fallDetectISR, RISING);
      
      Serial.println("Fall detection interrupt configured on GPIO 35");
      
      // Clear any pending interrupts
      clearADXL345Interrupt();
      
      // Load fall event count from RTC memory
      fallEventCount = rtcData.fallEventCount;
      Serial.print("Previous fall events: ");
      Serial.println(fallEventCount);
    }
  }
...
void loop() {
  // Check for fall detection interrupt
  if (fallDetected) {
    fallDetected = false;
...
void handleFallDetectedState() {
  ...
  fallEventCount++;
  rtcData.fallEventCount = fallEventCount;
  rtcData.lastFallTimestamp = millis();
  ...
  if (accelInitialized) {
    ...
    Serial.print(magnitude);
    Serial.print(" m/s² (");
    Serial.print(magnitude / 9.81);
    Serial.println("g)");
  }
  
  Serial.print("Fall Event Count: ");
  Serial.println(fallEventCount);
  Serial.print("Boot Count: ");
  Serial.println(rtcData.bootCount);
  Serial.println("========================================\n");
  
  // Display fall alert on OLED
  displayFallAlert();
  ...
  // Return to previous state or continue measurement
  if (fingerDetected && (millis() - measurementStartTime < MEASUREMENT_DURATION_MS)) {
    currentState = STATE_MEASURING;
    Serial.println("Resuming measurement...");
  } else {
    currentState = STATE_STORE_DATA;
    Serial.println("Proceeding to store data...");
  }
}
...
void scanI2CBus() {
  ...
  for (address = 1; address < 127; address++) {
    ...
    if (error == 0) {
      ...
      } else if (address == 0x53 || address == 0x1D) {
        Serial.print(" (ADXL345 Accelerometer)");
      }
      Serial.println();
      deviceCount++;
    }
...
void displayFallAlert() {
  ...
  display.setTextColor(SSD1306_WHITE);
  display.setCursor(0, 0);
  display.println(F("!!! FALL DETECTED !!!"));
  display.drawLine(0, 10, SCREEN_WIDTH, 10, SSD1306_WHITE);
  
  // Fall event count
  display.setCursor(0, 15);
  display.print(F("Event #"));
  display.println(fallEventCount);
  ...
  if (accelInitialized) {
    ...
    display.print(magnitude / 9.81, 1);
    display.println(F("g"));
  }
  ...
  display.setCursor(0, 50);
  display.setTextSize(1);
  display.println(F("Alert logged"));
